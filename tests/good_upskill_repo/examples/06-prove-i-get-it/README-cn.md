...
> 关键区别在 "历史". 普通文件夹里你改了文件, 旧版本就没了; repo 把每次改动都记成一个 commit 存起来, 随时能回看甚至回退. 见 [02-create-repo](../02-create-repo/README-cn.md) 里讲 repository 概念的部分.
...
> 存档的比喻点出两件事: 一是 commit 是离散的时间点 (不是连续自动保存), 二是你可以回到任意一个存档点. 每个 commit 都有作者, 时间和一句 message. 见 [02-create-repo](../02-create-repo/README-cn.md) 里把 commit 比作存档的部分.
...
> README 是别人打开你 repo 第一眼看到的介绍页, GitHub 会把它渲染在主页正中间. 对这门纯网页的课来说, 勾上 README 还有个实际好处: 让你有个文件可以马上练习编辑. 见 [02-create-repo](../02-create-repo/README-cn.md) 里讲初始化 README 的部分.
...
> 这门课建议用 public, 因为后面 Pull Request 的 review 过程在 public repo 里更直观可见, 也方便当作作品展示. 见 [02-create-repo](../02-create-repo/README-cn.md) 里对比 public 与 private 的部分.
...
> commit 的粒度直接决定历史的可读性. 一个理想的 commit 是 "一件语义上完整的小事". 见 [03-edit-files](../03-edit-files/README-cn.md) 里讲 commit 记录改动的部分.
...
> 半年后你或队友翻历史找 "哪次引入了这个改动", 靠的就是 message. 一句清楚的 message 是最省事的沟通. 见 [03-edit-files](../03-edit-files/README-cn.md) 里讲 commit message 的部分.
...
> 网页端的 web editor 让你完全不碰 git 也能增删改文件, 每次 Commit changes 就等于做了一个 commit. 见 [03-edit-files](../03-edit-files/README-cn.md) 里讲用 web editor 新建文件的部分.
...
> commit history 是项目的时间线, 点开任意一个 commit 还能看到那次具体改了哪些行. 这就是版本控制的核心价值: 一切改动有据可查. 见 [03-edit-files](../03-edit-files/README-cn.md) 里讲 commit history 的部分.
...
> 重点是 "隔离": branch 让未完成或有风险的改动待在自己的线上, 不污染大家共享的稳定版本. 见 [04-git-branch](../04-git-branch/README-cn.md) 里用另存副本类比 branch 的部分.
...
> 正因为 main 是共享基准, 现代协作的规矩才是 "不直接动 main, 改动先在 branch 上做". 见 [04-git-branch](../04-git-branch/README-cn.md) 里讲默认分支的部分.
...
> branch 把 "正在进行的工作" 和 "稳定的成品" 在物理上分开, 这是多人协作能并行推进的前提. 见 [04-git-branch](../04-git-branch/README-cn.md) 里讲为什么协作需要分支的部分.
...
> 新 branch 一开始是当前分支的完整副本, 之后你在它上面的 commit 都只留在这条线上. 见 [04-git-branch](../04-git-branch/README-cn.md) 里讲在 web 上建 branch 的部分.
...
> 这正是 branch 的意义所在: 隔离. 只有 merge 这个动作才会把 branch 的改动带回 main. 见 [04-git-branch](../04-git-branch/README-cn.md) 里讲改动不影响主干的部分.
...
> 把 "改动" 和 "正式采纳改动" 拆开, 正是 PR 的核心设计: 中间那道缝隙留给 review 和讨论. 见 [05-merge-branch](../05-merge-branch/README-cn.md) 里讲 Pull Request 是什么的部分.
...
> 这三样把一次改动的 "内容, 讨论, 决定" 集中在一个可追溯的页面上, 这也是为什么 PR 成了团队沟通的载体. 见 [05-merge-branch](../05-merge-branch/README-cn.md) 里列 PR 三个区域的部分.
...
> review 的价值在个人练习里不明显, 但在团队里是质量和知识传递的保障. 半年后 "这行为什么这么写" 的答案就在 PR 对话里. 见 [05-merge-branch](../05-merge-branch/README-cn.md) 里讲 code review 的部分.
...
> 方向 (base <- compare) 是关键: base 是要被合并进去的目标 (通常 main), compare 是你带改动的 branch. 见 [05-merge-branch](../05-merge-branch/README-cn.md) 里讲动手开 PR 的部分.
...
> merge 是把 branch 的 commit 正式并入 main 的动作, 之后这些改动成为项目正式版本的一部分. 见 [05-merge-branch](../05-merge-branch/README-cn.md) 里讲 merge 的部分.
...
> 需要的话删掉的 branch 之后还能恢复, 所以不用担心丢东西. 见 [05-merge-branch](../05-merge-branch/README-cn.md) 里讲合并后删 branch 的部分.
...
> 解决它就是人工挑一下留哪个再确认; 只要各改各的文件或及时同步 main, conflict 就很少见. 深入解决是以后的话题. 见 [05-merge-branch](../05-merge-branch/README-cn.md) 里讲 merge conflict 的部分.
