...
description: 给一个 showcase 教学仓库收尾定型. 重写 README-ORIGINAL (Lesson 门面), 写出根目录的 README 与 TICKET (仓库总览加整门课验收清单, 覆盖怎么学与学完怎么展示发布, 一次产出全部语种), 再跑 lesson-smith sync 生成 SYLLABUS 与快照, 最后跑 lesson-smith lint 把关. 创作流的最后一步, examples 与 forge 都做完之后用. 关键词, finalize showcase, 重写 README-ORIGINAL, 写根 README TICKET, 生成 SYLLABUS, showcase 收尾.
...
3. 跑 `lesson-smith sync` 生成 `docs/tasks/SYLLABUS` 与 `docs/tasks/01-showcase/` 快照.
...
跑 `lesson-smith sync` (它生成 `docs/tasks/SYLLABUS` 与 `docs/tasks/01-showcase/` 下的 README, TICKET 快照). 若命令不可用或报错, 如实告诉用户, 不要手写这些生成物.
