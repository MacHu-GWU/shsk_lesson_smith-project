...
description: "为当前 showcase 教学仓库锻造学习与展示工具链: 产出 docs/showcase 五份文档 (learn, runbook, quiz, demo, publish) 与四个子 skill (showcase-learn-cn, showcase-quiz-cn, showcase-demo-cn, showcase-publish-cn). 课程内容写完后手动跑一次."
...
- `ref/showcase/docs-showcase-learn-spec.md` — 写 `01-showcase-learn-cn.md` 的规范.
- `ref/showcase/docs-showcase-runbook-spec.md` — 写 `02-showcase-runbook.md` 的规范.
- `ref/showcase/docs-showcase-quiz-spec.md` — 写 `03-showcase-quiz-cn.md` 的规范.
- `ref/showcase/docs-showcase-demo-spec.md` — 写 `04-showcase-demo-cn.md` 的规范.
- `ref/showcase/docs-showcase-publish-spec.md` — 写 `05-showcase-publish-cn.md` 的规范 (自包含 publish 清单).
...
- `ref/showcase/showcase-learn-cn.SKILL.md`, `ref/showcase/showcase-quiz-cn.SKILL.md`, `ref/showcase/showcase-demo-cn.SKILL.md`, `ref/showcase/showcase-publish-cn.SKILL.md` — 四个子 skill 的近乎静态模板, 直接拷.
...
- **quiz 考法自定义**: `03-showcase-quiz-cn.md` 里对 showcase-quiz-cn 行为的特殊要求 (没有就按默认).
- **demo 排练自定义**: `04-showcase-demo-cn.md` 里对 showcase-demo-cn 行为的特殊要求 (默认听众, 时长偏好, 哪几幕重点练, 追问狠不狠; 没有就按默认). 顺便确认那份讲故事底稿是否偏离了默认七幕主线.
- **publish 口径**: `05-showcase-publish-cn.md` 里需要创作者兜底的取舍 (哪些 borderline 项默认保留还是问, commit plan 侧重, 有没有额外要删或要留的东西). 没有就按 spec 默认.
...
- `01-showcase-learn-cn.md` (依 docs-showcase-learn-spec): #1 学习素材 + #2 引导路径.
- `02-showcase-runbook.md` (依 docs-showcase-runbook-spec): 一次性 setup + 推进中操作, 把隐性步骤显式化.
- `03-showcase-quiz-cn.md` (依 docs-showcase-quiz-spec): 指向 Phase 2 定位到的题库真身, 加上考法自定义.
- `04-showcase-demo-cn.md` (依 docs-showcase-demo-spec): 指向 Phase 2 定位到的讲故事底稿, 记下默认七幕主线与排练自定义.
- `05-showcase-publish-cn.md` (依 docs-showcase-publish-spec): 自包含的 publish 清单. cardinal 删除的 glob 要对这个 repo 展开成真实文件清单; commit plan 要引这个 repo 的真实文件并按依赖排序; borderline 与敌意扫描规则照 spec 五块写全.
...
- `ref/showcase/showcase-learn-cn.SKILL.md` → `.claude/skills/showcase-learn-cn/SKILL.md`
- `ref/showcase/showcase-quiz-cn.SKILL.md` → `.claude/skills/showcase-quiz-cn/SKILL.md`
- `ref/showcase/showcase-demo-cn.SKILL.md` → `.claude/skills/showcase-demo-cn/SKILL.md`
- `ref/showcase/showcase-publish-cn.SKILL.md` → `.claude/skills/showcase-publish-cn/SKILL.md`
...
4. 告诉用户: 用 `/showcase-learn-cn` 开始学, `/showcase-quiz-cn` 自测, `/showcase-demo-cn` 排练讲法, 学完用 `/showcase-publish-cn` 发布; `docs/showcase/` 里哪里不对直接改, 或 `refresh <name>` 重生成一份.
...
- publish 清单 forge 只生成, 不执行: 真正的删除, 重写, 发布是学生后来跑 `/showcase-publish-cn` 时的事.
