...
你在 **upskill 创作工作流的第 9 到 10 步**: 补两头, 然后统稿. 这两步一起做, 因为它们要的是同一次通读.

主剧本是 `lesson-smith` skill 的 `ref/02-upskill/upskill-authoring-workflow.md`, 本步对应它的第 10 到 11 节. 先读那两节接上上下文, 再按下面动手.
...
- **quiz 也要一起统.** 题面用的术语要和教学 Task 对得上, 溯源链接要还活着. 它是分开写的, 最容易和主线分叉.
- **刚写的两头也要一起统.** 它们是最后写的, 最容易和主线用词不一致.
- **过不过由创作者说了算**, 不是你单方面宣布. 过了这一步教学内容才算定稿, 后面四步 (梳理时间, 锻造, 根目录文档, 出厂) 才拿得到稳定素材.
- **别关 session, 接着做梳理时间那一步.** 统稿刚通读完全系列, 下一步 (梳理各 Task 的预计用时) 要的正是同一次通读的记忆, 告诉创作者敲 `/lesson-smith-upskill-author-step-11-calibrate-time`.
