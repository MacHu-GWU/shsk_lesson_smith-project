---
name: lesson-smith-readup-author-step-01-to-02-scope
description: "第 1 到 2 步: 想清楚这门课教什么, 写 README-ORIGINAL 粗稿, 并把 examples 之外要学的东西先做完"
argument-hint: "[自由说明...]"
---

# readup 创作流 第 1 到 2 步

你在 **readup 创作工作流的第 1 到 2 步**: 定题. 把这门课教什么想清楚, 并把 `examples/` 之外的地基铺好.

主剧本是 `lesson-smith` skill 的 `ref/01-readup/readup-authoring-workflow.md`, 本步对应它的第 2 到 3 节. 先读那两节接上上下文, 再按下面动手.
...
- `ref/01-readup/readup-repo-layout.md`: readup 的目录结构与命名, 先读它对齐整体布局.
...
- **README-ORIGINAL 这一版是粗稿种子, 不要抠 description.** 第 10 步会拿定稿的 examples 当素材整份重写它. 现在内容还没写, 抠也抠不准.
...
- **别忘了建 `lm.json`**, 内容 `{"type": "readup"}`. 它是整仓唯一声明 repo 类型的地方, 缺了 lint 会短路成一条报错, 其余一项都不查.
- 只写 `-cn.md`.
