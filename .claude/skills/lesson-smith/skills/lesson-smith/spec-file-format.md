...
每个特殊文件的规范是一个**目录**, 里面每个语种两份文件 (一份 spec, 一份 template). 只有一处例外, 见第 1 节末尾.
...
**判据只有一条: 这份 spec 是不是在规定某一份产物的正文该怎么写.**

- **规定正文怎么写的分** (`02` 到 `06`, `11`, `12`, `14`, `13-forge-shared/` 下那几个 spec 目录, 以及特化层的 `<type>-readme-spec/`, `<type>-ticket-spec/`, `showcase-demo-*-spec/`). 受控词汇, 句式, 语感全都跟语种走, 各写各的才对; 理由见下一节.
- **只描述形状与流程的不分** (`01-repo-layout`, `07-syllabus-spec`, `08-series-converge-spec`, `09-root-docs-spec`, `10-ship-spec`, 以及各层的 layout 与 workflow). 它们说的是 "长什么形状" 与 "做什么", 换个语种照样成立.

**带 template 的必然落在分的那一侧**, 因为 template 本身就是那个语种的文本, 复制过去就变成中文文档. 但**反过来不成立**: `14-wrap-up-readme-spec/` 只有 spec 没有 template (它的骨架就是通用 Task README 那一份, 见第 1 节末尾), 它仍然规定正文该怎么写, 所以照样分语种. 早先那条 "带 template 的分, 不带的不分" 是这条判据的一个代理, `14` 进来之后就不准了.
