# docs/showcase/01-showcase-learn-cn.md 规范, 中文版

**适用范围**: showcase 类型 repo 的 `docs/showcase/01-showcase-learn-cn.md`, 由 `lesson-smith-showcase-forge` 生成, 给 `showcase-learn-cn` skill 消费.

**配套骨架**: [docs-showcase-learn-cn-template.md](docs-showcase-learn-cn-template.md).
...
`examples/01, 02 ...` 的渐进序列就是学生要走的路, `showcase-learn-cn` skill 按它往前推.
...
- **不要逐条复述每个 Task 的正文.**
- **走到教学 Task 为止.** 结尾那个 quiz Task 和 demo Task 分别归 `showcase-quiz-cn` 与 `showcase-demo-cn` 管, 不在带学路径里.
