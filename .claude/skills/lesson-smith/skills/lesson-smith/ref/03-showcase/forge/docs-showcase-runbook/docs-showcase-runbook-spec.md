# Spec for docs/showcase/02-showcase-runbook.md (English)

**Scope**: the file `docs/showcase/02-showcase-runbook.md` in an showcase repo. Written by `lesson-smith-showcase-forge`, read by the `showcase-learn` skill.

**Skeleton**: [docs-showcase-runbook-template.md](docs-showcase-runbook-template.md).

**Not generated at the moment**: forge emits the `-cn` set only, per [docs-showcase-runbook-cn-spec.md](docs-showcase-runbook-cn-spec.md). This spec and its skeleton stay put, ready for the multi-language module to pick up.
...
- It does not index. That is what `01-showcase-learn.md` is for.
