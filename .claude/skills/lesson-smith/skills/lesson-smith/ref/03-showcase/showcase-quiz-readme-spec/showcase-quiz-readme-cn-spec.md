# Showcase quiz Task 的 README 规范, 中文版

**适用范围**: 仅 showcase 类型 repo 里 quiz 那个 Task 的 `README-cn.md`, 目录固定命名 `examples/NN-prove-i-get-it`.
...
**配套骨架**: [showcase-quiz-readme-cn-template.md](showcase-quiz-readme-cn-template.md).
...
`docs/showcase/03-showcase-quiz-cn.md` 只是一层指过来的薄壳, `/showcase-quiz-cn` 抽题也是抽这里的题. 题不写在这份文件里, 那两处就是空的.

写这份文件时 AI 提建议, **创作者拍板定题**. 视角是学生自己检查自己 (所以目录叫 `prove-i-get-it`), 便于对外展示.

配套的 TICKET 另有规范, 见 [showcase-quiz-ticket-spec](../showcase-quiz-ticket-spec/showcase-quiz-ticket-cn-spec.md).
