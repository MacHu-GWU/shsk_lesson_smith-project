...
Always load your interaction base first: read `.claude/skills/showcase-quiz/ref/agent-skill-interaction-pattern.md` (bundled with this skill) and follow it. In short: lead at the opening, one question per turn, a confident-teacher tone, locate things by header or keyword, never by line number. Below is only what is specific to showcase-quiz.

## Knowledge sources (fixed, do not invent)

- Bank entry point: `docs/showcase/03-showcase-quiz.md` — it points to the actual question bank (the README of the quiz mini task) and records the human's quiz customization. Read it first, then follow its link to the bank.
...
- Context: `docs/showcase/01-showcase-learn.md` — when the learner gets one wrong and wants the source, or when generating fresh questions.
...
1. Read `docs/showcase/03-showcase-quiz.md`, follow the link to the bank, note the total number of questions, and read the human's quiz customization (honor it if present).
2. Offer two modes:
...
3. Recommendation: low means go back to `showcase-learn` for the affected part; high means try progressive or a fresh batch, or move on to `showcase-demo` to rehearse telling the story.
