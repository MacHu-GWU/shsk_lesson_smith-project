<!--
本文件是 showcase 类型 repo 的 examples/README 系列索引的规范加可直接套用的模板. 这段顶部注释只服务于创作; 产出真正的 examples/README-cn.md 时, 把整段注释删掉, 只留后面的正文.
...
适用范围: showcase 类型 repo 根目录下的 examples/README(-lang).md. 它不遵循 ref/readme-spec.md (那是给单个 task 的教学 README 用的).
...
showcase 里两个特殊 mini task 的摆放: quiz 环节 (NN-prove-i-get-it) 和 demo 环节 (how-i-build-this) 都在靠后的位置, demo 是 examples 的最后一个. 在索引里, 它们通常各自单独成组或并入结尾的 "收尾与展示" 组; 用一句话点明它们的性质 (自测题库, 讲故事底稿), 不要和前面的教学 mini task 混为一谈.
...
写作原则:
- 以 examples/README-cn.md 中文版为准: 先写中文, 全部定稿后随整门课一次性重写出英文 examples/README.md (见 [ref/rewrite-en-spec.md](../rewrite-en-spec.md)).
...
- 结尾一段小结: 把这几块串起来, 说清学完整条线能得到什么, 以及自测 (quiz) 与讲故事 (demo) 怎么收尾.
...
[一句话: 前面把该学的都学了, 这一组负责自测与对外展示.]

- [NN-prove-i-get-it](NN-prove-i-get-it/README-cn.md): 自测题库, 检验你对整门课是否知其然知其所以然.
- [ZZ-how-i-build-this](ZZ-how-i-build-this/README-cn.md): 讲故事底稿, 帮你把这段学习经历梳理成能对别人讲的作品叙事.

---

## 4. 小结

[把上面几组串起来, 说清学完整条线你能得到什么, 以及怎么用 quiz 自测, 用 demo 排练讲这段经历.]
