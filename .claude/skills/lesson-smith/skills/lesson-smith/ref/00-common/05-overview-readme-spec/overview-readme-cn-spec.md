...
**角色固定的 Task 各自单独成组.** 除了教学 Task, `examples/` 里还有两种角色固定, 性质不同的 Task: 排在主线之后收尾之前的特殊 Task (例如 showcase 的自测环节与讲故事环节), 以及排在最后的收尾 Task. 这类 Task **一个占一个 group**, 组里就它一条, 用一句话点明它的性质.
