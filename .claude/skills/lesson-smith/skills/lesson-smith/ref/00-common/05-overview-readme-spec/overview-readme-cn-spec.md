...
**什么时候写**: 不是第一个写的. 综述与主线 (02 往后) 全部写完之后, 回头补开头结尾时才写它. 地图只能在路修完之后画. 它写完了, 整个 examples 才算齐, 才轮到统稿.
...
**角色固定的 Task 各自单独成组.** 除了教学 Task, `examples/` 里还有两种角色固定, 性质不同的 Task: 排在主线之后收尾之前的特殊 Task (upskill 与 showcase 共有的自测环节, 以及 showcase 独有的讲故事环节), 以及排在最后的收尾 Task. 这类 Task **一个占一个 group**, 组里就它一条, 用一句话点明它的性质.

不要和教学 Task 混在同一条 bullet 列表里, 也不要几个挤进同一个组. **收尾 Task 三类都有**, 所以这一条对三类都成立, 哪怕某一类 (readup) 一个特殊 Task 都没有.

各位置分别是什么角色, 哪个类型有哪些特殊 Task, 见 [01-repo-layout.md](../01-repo-layout.md) 第 4.2 节.
...
**H1 里用 "系列索引" 这个说法, 不用 "索引 Task".** 规范里一律叫索引 Task, 那是给维护者的术语; H1 是学生打开这个 Task 第一眼看到的标题, 那里 "系列索引" 更像人话. template 里已经这么写好了.
...
- 指向兄弟 Task 的链接都带 `../`
- 读起来是分组和递进, 不是平铺罗列
- 特殊 Task 与收尾 Task 都各自单独成组, 没被混进教学 Task 的列表里
