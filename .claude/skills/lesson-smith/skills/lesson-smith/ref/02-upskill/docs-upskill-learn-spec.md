...
定性: 这是 "学习资料索引", 由 lesson-smith-upskill-forge 生成, 给 upskill-learn skill 消费. 它不铺开讲知识 (知识在 examples 和素材本身里), 只回答三件事: 要学的东西有哪些, 在哪, 按什么顺序过. 它是索引, 不是完整的知识清单.

适用范围: upskill 类型 repo 的 docs/upskill/01-upskill-learn.md. 这是给 AI/skill 看的元文件, 人类读的是真正的教程正文 (examples 下的), 不读这个; 所以它产出内容全英文, 不走 cn-first 铁律. 因此下面的正文模板本身就是英文的.
...
Part 2 引导路径 (guided path): examples/01, 02 ... 的渐进序列就是学生要走的路, upskill-learn skill 按它往前推. 这部分 forge 扫一遍 examples 目录就能得到, 容易. 这里只概述整条线怎么推进 (分成几段, 每段学到什么), 并点向 examples/README 和 docs/tasks/SYLLABUS; 对情况 B, 标明每一段路径覆盖 Part 1 的哪些素材. 不要逐条复述每个 example 的正文.
...
- [the tasks section of mise.toml](../../mise.toml) — [the common commands] — [for running things, see docs/upskill/02-upskill-runbook.md]

## 2. Guided Path

[The examples/01, 02 ... progression is the path the student walks. Summarize how the whole line advances; point at the index and the syllabus; do not restate each example.]
