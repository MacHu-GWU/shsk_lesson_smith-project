# Spec for docs/upskill/02-upskill-runbook.md (English)

**Scope**: the file `docs/upskill/02-upskill-runbook.md` in an upskill repo. Written by `lesson-smith-upskill-forge`, read by the `upskill-learn` skill.

**Skeleton**: [docs-upskill-runbook-template.md](docs-upskill-runbook-template.md).

**Not generated at the moment**: forge emits the `-cn` set only, per [docs-upskill-runbook-cn-spec.md](docs-upskill-runbook-cn-spec.md). This spec and its skeleton stay put, ready for the multi-language module to pick up.
...
- It does not index. That is what `01-upskill-learn.md` is for.
