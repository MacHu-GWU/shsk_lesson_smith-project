...
Read `.claude/skills/upskill-learn/ref/agent-skill-interaction-pattern.md` first, bundled with this skill, and work the way it says. The short version: lead at the opening, then go where the learner goes; one question at a time; short and specific; locate things by header or keyword, never by line number. Everything below is what is particular to this skill.

## Where your knowledge comes from (fixed, do not invent)

- `docs/upskill/01-upskill-learn.md`. The index: what there is to study, and how the tasks progress.
- `docs/upskill/02-upskill-runbook.md`. The runbook: what to set up first, and what to run along the way.
- The material itself: the READMEs of the tasks under `examples/`, plus whatever files the index points at. **Open the real file before you teach from it.** Paraphrasing the index is not teaching.
...
This is the English edition. Read the English docs under `docs/upskill/` and teach from the English files they link to.

Every language has its own skill and its own set of docs. If the learner would rather work in another language, point them at that skill (`/upskill-learn-cn`, for instance) instead of translating as you go.
...
Open by reading the first two sections of `docs/upskill/01-upskill-learn.md`. **Do not dump the whole file.** Work out the mode (an explicit argument wins; a named file or a question means Context-dive; "what next" means Next-step; "carry on" means Resume; anything else means Orient), confirm it in a line, and begin.
...
Read `docs/upskill/notes/learn-progress.md` if it exists. Ask: "Last time we stopped at X. Carry on from there, or switch to something else?"
