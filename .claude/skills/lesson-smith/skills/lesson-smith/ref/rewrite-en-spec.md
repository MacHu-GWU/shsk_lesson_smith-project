...
- 值保持一行, 不含换行.
- 值必须用双引号包起来.
- 值里不许出现任何引号类字符 (单引号, 双引号, 反引号). **这一条包括英文的撇号**: `GitHub's`, `It's`, `interviewer's` 全部会被 lint 抓. 所有格换成 of 结构或者换个说法, 缩写在这里一律写全. 这一条只管 frontmatter, 正文里的缩写不受限, 而且是被鼓励的.
...
**`README-ORIGINAL` 的 description 另有一个下限, 英文 400 字符** (中文 200). 上面那条 "不要为了凑短而砍内容" 在别的文件上是提醒, 在这一份上是硬要求: 它是整门课进 org 级索引的门面, 而实测下来英文版最容易出的问题恰恰是**被压缩**. 同一段内容用英文写出来天然要长一截, 如果产出的英文只比中文长一点点, 那基本可以断定它是被压过的, 回去扩写. 扩写是这一份的任务本身, 不是可选项. 各字段的完整口径见 [ref/common/readme-original-spec/readme-original-spec.md](common/readme-original-spec/readme-original-spec.md).
...
**而且这一条不该只靠事后自觉, 它该由 lint 直接比对目录名.** 实测六份语料里有两份的英文 H1 被砍掉了 `-project` 后缀, 这正是重写模型觉得后缀多余时最容易做的手脚. 它是静默的, 事后抽查时很容易看漏一份. 提示词和事后修都是软的, 只有 lint 拿目录名逐字节比一次才是硬的.
