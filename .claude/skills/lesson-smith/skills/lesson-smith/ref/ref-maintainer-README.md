...
- **`00-common` 的判据是 "被多个 repo type 共享", 不是 "四类都有".** 比如 `05` 与 `06` 那个索引 Task 只有三类有, `11` 到 `13` 更只有 upskill 与 showcase 两类有, 但共享就够格放这儿. 各文件自己的适用范围写在各自开头. `14` 那个收尾 Task 和 `05`, `06` 一样是三类都有.
- **共享的代价是占位符.** `11` 到 `13` 那几份里的类型名写成 `{{TYPE}}`, 落地时替换. 换来的是一份规范只有一个副本; 漏换可以机械查出来 (产出里 grep `{{TYPE}}` 必须 0 命中), 而漂移查不出来. `14` 三类通用, 正文里不出现类型名, 所以不带占位符.
...
**分不分语种的判据: 这份 spec 是不是在规定某一份产物的正文该怎么写.** 是就分 (受控词汇, 句式, 语感全都跟语种走, 而且用中文描述英文该怎么写正是 chinglish 最大的来源); 只描述形状与流程的不分, 所以 `01`, `07`, `08`, `09`, `10` 那几份单文件都不带语种后缀.

带 template 的必然分. **反过来不成立**: `14` 那个收尾 Task 只有 spec 没有 template (它的骨架就是通用 Task README 那一份), 但它规定正文该怎么写, 所以照样分语种. 完整理由 (以及为什么不再把规范塞进顶部注释) 见 skill 根目录的 `spec-file-format.md`.
