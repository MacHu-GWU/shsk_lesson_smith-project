...
- **README-ORIGINAL 这一版是粗稿种子, 不要抠 description.** 第 13 步会拿定稿的 examples 当素材整份重写它. 现在内容还没写, 抠也抠不准.
...
- **别忘了建 `lm.json`**, 内容 `{"type": "upskill"}`. 它是整仓唯一声明 repo 类型的地方, 缺了 lint 会短路成一条报错, 其余一项都不查.
- 只写 `-cn.md`.
