...
readup 是纯阅读型仓库: 不带任何 AI 学习工具链, 也没有任何特殊 Task, 所以它比 upskill 和 showcase 少一个锻造阶段, 也少了写 quiz 那一段. 一共 11 步 7 个阶段.
...
**二, 读主剧本** `ref/01-readup/readup-authoring-workflow.md`. 它是 11 步的骨架和阶段划分表.
...
| 主线写完了, 但 `examples/01-*/` 这个索引 Task 或最后那个收尾 Task 还缺 | 第 7 步 |
| 两头都在, 根目录缺 `README-cn.md` 或 `TICKET-cn.md` | 第 8 到 10 步. **直接问创作者统稿和时间梳理做没做**, 这两步从文件系统看不出来 |
| 根目录三份 cn 齐了, 但没有 `docs/tasks/SYLLABUS-cn.md` | 第 11 步 |
...
/lesson-smith-readup-author-step-03-to-05-plan-and-trial
/lesson-smith-readup-author-step-06-mainline
/lesson-smith-readup-author-step-07-to-08-bookends-and-converge
/lesson-smith-readup-author-step-09-calibrate-time
/lesson-smith-readup-author-step-10-root-docs
/lesson-smith-readup-author-step-11-ship
```

后面几条各建议开一个新 session, 因为它们都要通读整门课, 而写作阶段留下的上下文只会挤占注意力. 创作者在新 session 里会重新敲你一次, 这是预期行为, 不是重复劳动.

**第 9 步是唯一一条不要另开 session 的**: 它接着第 8 步 (统稿) 做, 用的是统稿时刚通读全系列的那份记忆.
...
- **`examples/` 的位置约定**: 01 索引, 02 综述, 03 往后主线, 最后一个收尾. readup 是三类里唯一没有特殊 Task 的. 完整约定见 `ref/00-common/01-repo-layout.md` 第 4.2 节.
