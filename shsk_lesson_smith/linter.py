...
def rule_estimated_hours(repo: Repo) -> "list[CheckResult]":
    """lm.json's time budget must equal the sum of the task snapshots' estimates.

    ``estimated_hours_lower`` / ``estimated_hours_upper`` are generated by
    ``lesson-smith sync``, so like the SYLLABUS they are linted for drift rather
    than for style: re-derive the sum and compare. The usual failure is
    re-estimating one task and not re-running sync.

    A missing or invalid manifest is not reported here; ``rule_manifest`` already
    owns that and repeating it would double every report.
    """
    ...
    def _check() -> None:
        ...
        if not estimate.is_complete:
            ...
            raise LintError(
                f"No usable estimated time in the TICKET of: {names}. "
                f"Each TICKET needs a line like "
                f"'{ESTIMATED_TIME_LABEL} 15 到 30 分钟' in minutes."
            )
        expected = (estimate.lower_hours, estimate.upper_hours)
        actual = (metadata.estimated_hours_lower, metadata.estimated_hours_upper)
        if actual != expected:
            raise LintError(
                f"lm.json estimated time is {actual[0]} to {actual[1]} hours, but "
                f"the task TICKETs sum to {expected[0]} to {expected[1]} "
                f"({estimate.lower_minutes} to {estimate.upper_minutes} minutes). "
                f"Re-run lesson-smith sync."
            )
